...
export class AddressService{
  ...
  async get(user: User, request: GetAddressRequest): Promise<AddressResponse> {
    ...
    await this.contactService.checkContactMustExist(user.username, getRequest.contact_id)
  ...
  async update(user: User, request: UpdateAddressRequest): Promise<AddressResponse> {
    const updateRequest: UpdateAddressRequest = this.validationService.validate(AddressValidation.UDPATE, request)
